...
class OperationService(BaseService):
    ...
    async def run(self, op_id):
        ...
        try:
            ...
            continue_operation = operation[0]['host_group']
            total_phases = 0
            all_hosts = []
            # initialize hosts with the agents that exist at the beginning

            while continue_operation is not None:
                hosts = continue_operation
                all_hosts = all_hosts + hosts
                self.log.debug(f"Running with {len(hosts)} hosts")
                # update previous hosts to include the agents that are running now and the agents that ran before
                for phase in operation[0]['adversary']['phases']:
                    ...
                    total_phases += await planner.execute(phase, hosts)
                    ...
                    # update hosts by checking if there are any new hosts
                continue_operation = await planner.find_host_diff(all_hosts, total_phases)
                self.log.debug(f"Continuing is: {continue_operation}")
