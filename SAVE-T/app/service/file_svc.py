...
class FileSvc(BaseService):
    ...
    async def download(self, request):
        ...
        """
        try:
            self.log.debug(f"Got download request with headers: {request.headers.get('file')} and {request.headers.get('platform')}")
    ...
    async def _go_compile(self, name, platform):
        ...
        if which('go') is not None:
            ...
            if arch is not None:
                self.log.debug("Compiling sandcat for arm")
                os.system('GOOS=%s GOARCH=%s GOARM=5 go build -o %s -ldflags="-s -w" %s' % (platform, arch, output, file_path))
            else:
