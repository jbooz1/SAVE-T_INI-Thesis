...
func RunCommand(command string, payloads []string, platform string, executor string, msfmux *sync.Mutex) (string, []byte, string) {
	...
	if len(missingPaths) == 0 {
		if strings.Contains(cmd, "metasploit") {
			msfmux.Lock()
			fmt.Printf("[*] Running Metasploit Command %s\n", cmd)
...
func runShellExecutor(executor string, platform string, command string) ([]byte, string) {
	...
	command = strings.Replace(command, "./", "/tmp/", -1)
