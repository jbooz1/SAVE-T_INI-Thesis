...
if __name__ == '__main__':
    ...
    with open('conf/%s.yml' % args.environment) as c:
        ...
        logging.debug("Starting MSFRPC with Subprocess")
        Popen(["msfrpcd", "-P", "password", "-S "], stderr=DEVNULL, stdout=DEVNULL)
        logging.debug("Serving Sandcat on Python Web Server")
        Popen(["python3", "-m", "http.server", "-d", f"{os.getcwd()}/plugins/sandcat/payloads"], stderr=DEVNULL, stdout=DEVNULL)
